...
using namespace std;
using namespace cv;
void preprocessing(cv::Mat& img, float offset,float gainR, float gainB);
...
void demosaic_nona(cv::Mat& Bayer, cv::Mat& Dst, int flag1 = 1, int flag2 = 1, int flag3 = 1, double k = 1.0, float sigma = 1.0) {
	...
	switch (flag1) {
	case 1:
		rawH = method1_h(Src1ch);
		rawV = method1_v(Src1ch);
		break;
	case 2:
		rawH = method2_h(Src1ch);
		rawV = method2_v(Src1ch);
		break;
	case 3:
		rawH = method3_h(Src1ch);
		rawV = method3_v(Src1ch);
		break;
	case 4:
		rawH = method4_h(Src1ch,k);
		rawV = method4_v(Src1ch,k);
		break;
	default:
		std::cerr << "Wrong Linear Interpolation Flag" << endl;
		return;
	} //original matlab uses'replicate' for all filter
	...
	switch (flag2) {
	case 1:
		H_diff_gradient = diffilter_H_method1(dif_H);
		V_diff_gradient = diffilter_V_method1(dif_V);
		break;
	case 4:
		H_diff_gradient = diffilter_H_method4(dif_H,k);
		V_diff_gradient = diffilter_V_method4(dif_V,k);
		break;
	default:
		std::cerr << "Wrong Difference Filter Flag" << endl;
		return;
	}
	...
	switch (flag3) {
	case 1:
		residualR = bilinearfilter_method1(residualR);;
		break;
	case 4:
		residualR = bilinearfilter_method4(residualR,k);
	}
	...
	switch (flag3) {
	case 1:
		residualB = bilinearfilter_method1(residualB);;
		break;
	case 4:
		residualB = bilinearfilter_method4(residualB, k);
	}
